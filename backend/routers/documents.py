...
# Basic goal is not to like keep waiting after upload it will say upload done and this all
# things will happen in background
async def _background_ingest(user_id:str,namespace:str, filename:str,content:bytes,doc_id:str):
    """
        Run ingestion i n backgorund so upload endpoint returns immediately.
        Args:
            user_id(str) : id of user 
            namespace(str) : namespace of user 
...
async def upload_document(
    ...
    if not allowed:
        raise HTTPException(
            status_code=429,
            detail = f"Daily upload limit of {50} files reached. Upgrade to premium plan"
        )
    ...
    if ext not in ALLOWED_TYPES:
        raise HTTPException(
            status_code=400 , detail=f"Unsupported file .'{ext}. Allowed only {', '.join(ALLOWED_TYPES)}"
        )

    # Read and size check
    content = await file.read()
    if len(content) > MAX_FILE_SIZE:
        raise HTTPException(status_code=413 , detail="File exceed 20MB Limit")
    ...
    # Kick off the background task of pinecone with embeddings 
...
async def get_document(doc_id : str , current_user : User = Depends(get_current_user)):
    """
        get a particular document based on the document id 
        Args:
            doc_id(str) : Id of the document 
        Returns :
...
async def remove_document(doc_id : str , current_user : User = Depends(get_current_user)):
    """
        Remove a document based on the document id pass in the argument
        Args:
